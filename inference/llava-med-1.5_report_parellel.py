...
def eval_model(args):
    ...
    for line in tqdm(dataloader):
        ...
        if "reference_reports" not in line:
            ...
            # cur_prompt = qs
            appendix_1=f"You are a professional ophthalmologist. You are provided with a fundus image and {topk} reference report(s): "
            appendix_2="Please generate a report based on the image. It should be noted that the diagnostic information in the reference reports cannot be directly used as the basis for diagnosis, but should only be used for reference and comparison. Please only include the content of the report in your response."
