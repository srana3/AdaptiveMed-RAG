...
output_root=/home/wenhao/Project/intern/kangyu/annotations+scripts+outputs/annotations+scripts/med-dpo/harvard/retrieve/vqa/${data_type}/


if [[ -n "$clip_threshold" && -z "$fixed_k" ]]; then
    output_path="${output_root}/harvard_vqa_${data_type}_withReport_ConfTopk_clipThreshold-${clip_threshold}.jsonl"
elif [[ -n "$clip_threshold" && -n "$fixed_k" ]]; then
    output_path="${output_root}/harvard_vqa_${data_type}_withReport_fixedK-${fixed_k}_clipThreshold-${clip_threshold}.jsonl"
elif [[ -z "$clip_threshold" && -n "$fixed_k" ]]; then
    output_path="${output_root}/harvard_vqa_${data_type}_withReport_fixedK-${fixed_k}.jsonl"
elif [[ -z "$clip_threshold" && -z "$fixed_k" ]]; then
    output_path="${output_root}/harvard_vqa_${data_type}_withReport_ConfTopk.jsonl"
fi
...
cmd="CUDA_VISIBLE_DEVICES=2 python /home/wenhao/Project/intern/kangyu/annotations+scripts+outputs/annotations+scripts/med-dpo/harvard/retrieve/retrieve_clip_VQA_harvard.py \
    ...
    --eval_json /home/wenhao/Project/intern/kangyu/annotations+scripts+outputs/annotations+scripts/med-dpo/harvard/gpt_vqa/vqa/harvard_${data_type}_vqa.jsonl \
